...
// This first group of errors are generated by the blahtex core.

    make_pair(L"NonAsciiInMathMode",        // FIX: describe which character is the problem
...
// Now we have errors which may be generated by the command-line application (i.e. by main.cpp)
    ...
    make_pair(L"CannotRunConvert",
        L"Cannot run convert"),

    make_pair(L"CannotChangeDirectory",
        L"Cannot change working directory"),
};

wishful_hash_map<wstring, wstring> gEnglishMessagesTable(gEnglishMessagesArray, END_ARRAY(gEnglishMessagesArray));

wstring GetErrorMessage(const blahtex::Exception& e)
{
    wishful_hash_map<wstring, wstring>::const_iterator messageLookup = gEnglishMessagesTable.find(e.mCode);
